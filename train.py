...
@click.option('--label-noise', help='Label noise probability/percentage', metavar='FLOAT', type=click.FloatRange(min=0),
...
@click.option('--workers', help='DataLoader worker processes', metavar='INT', type=click.IntRange(min=1), default=6,
              show_default=True)
# I/O-related.
...
def main(**kwargs):
    ...
    # Handle label noise parameter (allow 10 for 10% or 0.1 for 10%)
    label_noise = opts.label_noise
    if label_noise > 1.0:
        label_noise /= 100.0

    dataset_class = 'training.dataset.ImageFolderDataset'
    if 'stl10' in opts.data.lower():
        dataset_class = 'training.dataset.STL10Dataset'
        dist.print0(f'Detected STL-10 dataset request from --data={opts.data}. Using training.dataset.STL10Dataset.')

    c.dataset_kwargs = dnnlib.EasyDict(class_name=dataset_class, path=opts.data,
    ...
    if opts.arch == 'ddpmpp':
        ...
        # c.network_kwargs.update(channel_mult_noise=1, resample_filter=[1, 1], model_channels=48,
        #                         channel_mult=[1, 1, 1])  # 尝试将参数减少一半
