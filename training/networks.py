    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None, return_features=False):
        ...
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                skips.append(x)
                if return_features and isinstance(block, UNetBlock):
                    features[f'enc_{enc_idx}'] = x.clone()
                    enc_idx += 1
        ...
        for name, block in self.dec.items():
            if 'aux_up' in name:
                ...
                x = block(x, emb)
                if return_features and isinstance(block, UNetBlock):
                    features[f'dec_{dec_idx}'] = x.clone()
                    dec_idx += 1
...
class DhariwalUNet(torch.nn.Module):
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None, return_features=False):
        ...
        # Store intermediate features if requested
        encoder_features = [] if return_features else None
        decoder_features = [] if return_features else None
...
class VPPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, return_features=False, **model_kwargs):
        ...
        # Call the underlying model with return_features parameter
        model_output = self.model((c_in * x).to(dtype), c_noise.flatten(), class_labels=class_labels,
                                  return_features=return_features, **model_kwargs)

        if return_features:
            F_x, features = model_output
...
if __name__ == "__main__":
    ...
    model = SongUNet(img_resolution=32,in_channels=3,out_channels=3,
                 ...
                 resample_filter=[1, 1])
    device = "cuda:0"
    x = torch.randn(1, 3,32, net.img_resolution, device=device)
    sigma = torch.tensor([0.5], device=device, dtype=torch.float32)
    flops, params_mb = compute_model_flops_and_params(model, (x, sigma))
    print(flops, params_mb)